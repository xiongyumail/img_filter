...
def main(args):
    ...
    if args.input_json:
        try:
            ...
            raise Exception(f"The file {args.input_json} was not found.")
        except json.JSONDecodeError:
            raise Exception(f"Failed to decode the JSON file {args.input_json}.")
    elif args.folder_path:
        folder_paths = args.folder_path
    else:
        raise Exception("No folder paths were provided. Please specify either --input_json or folder_path.")
...
if __name__ == "__main__":
    ...
    # 检查是否未提供输入参数，启动GUI
    if not args.input_json and not args.folder_path:
        try:
            ...
            print("Error: Tkinter is required to run the GUI. Please install Tkinter or provide input arguments.")
            sys.exit(1)
    else:
        main(args)